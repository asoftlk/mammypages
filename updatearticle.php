...
if(mysqli_num_rows($artupdate)>0){
			While($artrow= mysqli_fetch_array($artupdate)){
				...
				foreach($articleid as $value)
				{
					...
					if($key === false)
					{
					if(file_exists("admin/posts/".$artrow['image_name'])){
						unlink("admin/posts/".$artrow['image_name']);
					...
					else
					{
						...
						$target = $postingid.$key.$time.".".$ext;
								if (in_array($articleimage, ['jpg', 'png', 'jpeg'])) {
									...
									move_uploaded_file($_FILES['articleimage']['tmp_name'][$key], "admin/posts/".$target);
									}
 ...
 for ($x = 0; $x < count($articleimages); $x++) {
            ...
            //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
                if (in_array($articleimage, ['jpg', 'png', 'jpeg'])) {
                    ...
                    move_uploaded_file($_FILES['articleimages']['tmp_name'][$x], "admin/posts/".$target);
                    }
            }
            else{
                $articleimage="";
	...
	if($articlefile != NULL){
		...
		move_uploaded_file($_FILES['articlefile']['tmp_name'], "admin/posts/".$articletarget);
	}
	else{
		$articletarget=null;

$updatequery = "UPDATE posts SET language='$language', category='$category', article_title='$articleTitle', arttype='$arttype', pubtype='$pubtype', pub_datetime='$pub_datetime', tags='$tags', keywords='$keywords', description='$articledescription', datetime =now() ";	
 $featuredimage = $_FILES['featuredimage']['name'];
            if($featuredimage != ""){
				...
				if($imagename['featured_image'] && file_exists("admin/posts/".$imagename['featured_image'])){
					unlink("admin/posts/".$imagename['featured_image']);
				}
                ...
                //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
                    if (in_array($featuredimage, ['jpg', 'png', 'jpeg'])) {
                        ...
                        move_uploaded_file($_FILES['featuredimage']['tmp_name'], "admin/posts/".$featuretarget);
                        }
            ...
            if($articlefile != ""){
				...
				if($filename['articlefile'] && file_exists("admin/posts/".$filename['articlefile'])){
					unlink("admin/posts/".$filename['articlefile']);
				}
                ...
                //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
                    if (in_array($articlefile, ['pdf'])) {
                        ...
                        move_uploaded_file($_FILES['articlefile']['tmp_name'], "admin/posts/".$filetarget);
                        }
