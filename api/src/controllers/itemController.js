import {
  ...
  getDoctorsBySearchService,
  getMedicalClinicsBySearchService,
  getStudiosBySearchService,
  getBeautySalonsBySearchService,
  getPharmaciesBySearchService,
...
export const getDoctorsBySearch = async (req, res) => {
  try {
    const doctorToSearch = { ...req.body };

    const { statusCode, data, message } = await getDoctorsBySearchService(
      doctorToSearch
    );
...
export const getMedicalClinicsBySearch = async (req, res) => {
  try {
    const medicalClinicToSearch = { ...req.body };

    const { statusCode, data, message } =
      await getMedicalClinicsBySearchService(medicalClinicToSearch);
...
export const getStudiosBySearch = async (req, res) => {
  try {
    const studioToSearch = { ...req.body };

    const { statusCode, data, message } = await getStudiosBySearchService(
      studioToSearch
    );
...
export const getBeautySalonsBySearch = async (req, res) => {
  try {
    const beautySalonToSearch = { ...req.body };

    const { statusCode, data, message } = await getBeautySalonsBySearchService(
      beautySalonToSearch
    );
...
export const getPharmaciesBySearch = async (req, res) => {
  try {
    const pharmacyToSearch = { ...req.body };

    const { statusCode, data, message } = await getPharmaciesBySearchService(
      pharmacyToSearch
    );
...
export const getAllAdvertisements = async (req, res) => {
  try {
    ...
    const skip = req.body.skip
      ? req.body.skip
      : Number(process.env.DEFAULT_SKIP);
    const { statusCode, data, message } = await getAllAdvertisementsService(
      limit,
      skip
    );
    res.status(statusCode).json({ data, message });
...
export const addNewAdvertisement = async (req, res) => {
  try {
    let advertisementImage = [];

    if (isFilesObject(req.files) && req.files["imageUrl"]) {
      advertisementImage = req.files["imageUrl"];
    }

    const advertisementToAdd = { ...req.body };

    const { statusCode, message } = await addNewAdvertisementService(
      advertisementToAdd,
      advertisementImage
    );
...
export const updateAdvertisement = async (req, res) => {
  try {
    ...
    const { statusCode, message } = await updateAdvertisementService(
      id,
      pharmacyDataToEdit
    );
    res.status(statusCode).json({ message });
...
export const deleteAdvertisement = async (req, res) => {
  try {
    ...
    const { statusCode, message } = await deleteAdvertisementService(id);
    res.status(statusCode).json({ message });
  } catch (err) {
    console.log(err);
    return res
