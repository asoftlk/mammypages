...
export const getDoctorsBySearchService = async (doctorToSearch) => {
  try {
    let doctorsQuery = {};

    if (Object.keys(doctorToSearch).length > 0) {
      doctorsQuery = {
        where: {
          ...Object.keys(doctorToSearch).reduce((acc, key) => {
            acc[key] = {
              contains: doctorToSearch[key],
            };
            return acc;
          }, {}),
        },
    ...
    const allDoctors = await prisma.doctor.findMany(doctorsQuery);
...
export const getMedicalClinicsBySearchService = async (
  medicalClinicToSearch
) => {
  try {
    let medicalClinicQuery = {};

    if (Object.keys(medicalClinicToSearch).length > 0) {
      medicalClinicQuery = {
        where: {
          ...Object.keys(medicalClinicToSearch).reduce((acc, key) => {
            acc[key] = {
              contains: medicalClinicToSearch[key],
            };
            return acc;
          }, {}),
        },
    ...
    let allMedicalClinics;

    if (medicalClinicQuery.where?.services) {
      const documents = await prisma.clinic.findMany({
        where: {
          services: {
            hasSome: [medicalClinicToSearch.services],
          },
        },
      });

      allMedicalClinics = documents;
    } else {
      allMedicalClinics = await prisma.clinic.findMany(medicalClinicQuery);
    }
...
export const getStudiosBySearchService = async (studioToSearch) => {
  try {
    let studiosQuery = {};

    if (Object.keys(studioToSearch).length > 0) {
      studiosQuery = {
        where: {
          ...Object.keys(studioToSearch).reduce((acc, key) => {
            acc[key] = {
              contains: studioToSearch[key],
            };
            return acc;
          }, {}),
        },
    ...
    if (studiosQuery.where?.services) {
      const documents = await prisma.studio.findMany({
        where: {
          services: {
            hasSome: [studioToSearch.services],
          },
        },
      });

      allStudios = documents;
    } else {
      allStudios = await prisma.studio.findMany(studiosQuery);
    }
...
export const getBeautySalonsBySearchService = async (beautySalonToSearch) => {
  try {
    let beautySalonsQuery = {};

    if (Object.keys(beautySalonToSearch).length > 0) {
      beautySalonsQuery = {
        where: {
          ...Object.keys(beautySalonToSearch).reduce((acc, key) => {
            acc[key] = {
              contains: beautySalonToSearch[key],
            };
            return acc;
          }, {}),
        },
    ...
    if (beautySalonsQuery.where?.services) {
      const documents = await prisma.salon.findMany({
        where: {
          services: {
            hasSome: [beautySalonToSearch.services],
          },
        },
      });

      allBeautySalons = documents;
    } else {
      allBeautySalons = await prisma.salon.findMany(beautySalonsQuery);
    }

    return {
      statusCode: 200,
      data: allBeautySalons,
      message: "Successfully fetched",
    };
  } catch (err) {
    console.log(err);
...
export const getPharmaciesBySearchService = async (pharmacyToSearch) => {
  try {
    let pharmaciesQuery = {};

    if (Object.keys(pharmacyToSearch).length > 0) {
      pharmaciesQuery = {
        where: {
          ...Object.keys(pharmacyToSearch).reduce((acc, key) => {
            acc[key] = {
              contains: pharmacyToSearch[key],
            };
            return acc;
          }, {}),
        },
    ...
    if (pharmaciesQuery.where?.services) {
      const documents = await prisma.pharmacy.findMany({
        where: {
          services: {
            hasSome: [pharmacyToSearch.services],
          },
        },
      });

      allPharmacies = documents;
    } else {
      allPharmacies = await prisma.pharmacy.findMany(pharmaciesQuery);
    }
...
export const getAllAdvertisementsService = async (limit, skip) => {
  try {
    const allAdvertisements = await prisma.advertisements.findMany({
      take: limit,
      skip: skip,
    });
...
export const addNewAdvertisementService = async (
  advertisementToAdd,
  advertisementImage
) => {
  try {
    let advertisementImageFileObj = [];

    if (advertisementImage.length != 0) {
      advertisementImageFileObj = await ImageUploadFunc(
        advertisementImage,
        "advertisements"
      );
    }

    const updatedAdvertisementToAdd = {
      ...advertisementToAdd,
      imageUrl: advertisementImageFileObj[0],
    };

    await prisma.advertisements.create({
      data: updatedAdvertisementToAdd,
    });
...
export const updateAdvertisementService = async (id, pharmacyDataToEdit) => {
  try {
    const updateData = {};
