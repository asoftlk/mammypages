...
<div class="row m-0 desktopviewArticle">
   ...
   <div class="col-md-6">
			<?php $type = $_GET['type'];
			$hospitalid = $_GET['id'];
					$hospital =mysqli_query($conn, "SELECT * FROM hospital WHERE hospital_id= '$hospitalid'");
					...
					echo '<div class="row fillbg">
							...
							<img src="directory/hospital/'.$row["image"].'" class="img-fluid mb-2 mt-2" style="width:100%; max-height:250px">
							...
							<div class="row fillbg">
							    <div class="col-md-3"><img src="directory/hospital/'.$row["logo"].'" class="img-fluid" style="max-height:60px"></div>
							    <div class="col-md-4 p-0">
							        <p class="hosname mb-0">'.$row['name'].'</p>
							        <p class="">'.$speciality.'</p>
							    </div>
							    <div class="col-md-2 pl-0 text-center">';
							   // echo "SELECT *  FROM mp_comments WHERE userid='$userid' AND mp_id='$row[hospital_id]' AND follow_status=1";
							    $followstatus= mysqli_query($conn, "SELECT *  FROM mp_comments WHERE userid='$userid' AND mp_id='$row[hospital_id]' and follow_status=1");
							        ...
							        echo '<p class="mt-1" style="font-size:13px; font-weight:bold">'.$totalfollowers.' Followers</p>';
			                            ...
			                            echo '<p class="mt-1" style="font-size:13px; font-weight:bold"></p>';
			                        }
							    echo '</div>
								...
								echo '<div class="row fillbg mt-1">
								        <h5 class="heading mt-1" style="font-weight:bold; font-size:12px">ABOUT HOSPITAL';
								        
								        $galquery= mysqli_query($conn, "SELECT image_name FROM mpgallery WHERE hospitalid= '$hospitalid' and image_name!=''");
			...
			<div class="row p-2 fillbg" style="text-align:end; flex-flow: wrap">
			...
			<textarea name="reviewdata" style="width:100%; height:4rem; resize:none; border:1px solid #C7C7C7"></textarea><br style="clear:both">
			<div class="rate">
			...
			while($reviewrow= mysqli_fetch_array($fetch)){
				echo '
					<div class="row fillbg mt-2" style="padding: 10px; border-radius: 10px;">
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="padding:0 1rem">
        <h5 class="modal-title" id="galleryModalLabel">Hospital Gallery</h5>
...
<div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:980px;height:480px;overflow:hidden;visibility:hidden;">
        ...
        <div data-u="slides" style="cursor:default;position:relative;top:0px;left:0px;width:980px;height:380px;overflow:hidden; text-align:center">
				<?php
					$galquery = mysqli_query($conn, "SELECT * FROM mpgallery WHERE hospitalid= '$hospitalid'");
...
<div class="modal fade" id="video" tabindex="-1" role="dialog" aria-labelledby="videoModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="padding:0 1rem">
        <h5 class="modal-title" id="videoModalLabel">Hospital Video</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
 ...
 function removeReg(data, status) {
  Swal.fire({
    ...
    .then(function(value) {
		...
		if(data == 'Please login to Review the hospital'){
...
$(document).on('click', '#btn_more', function(){  
	   ...
	   $.ajax({  
			url:"ajax/reviewlist",  
			method:"POST",  
			data:{count:count, type:"morereviews", i:i, reviewid:"<?php echo $hospitalid; ?>"},  
			dataType:"text",  
			success:function(data)  
			{  
