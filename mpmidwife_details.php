		...
		<div class="container-fluid p-0">
			<div class="row m-0 desktopviewArticle">
					...
					<div class="left-menu-part" style="background:transparent;">
						<div class="unscroll" style="background-color:#fff; margin:10px 0; padding:10px 0">
							...
							<ul style="list-style-type:none; padding-left:0">
								<li><a role="button" href="midwife"><i class="icofont-midwife"></i> midwifeS</a></li>
				...
				<div class="col-md-6">
					<?php $type = $_GET['type'];
						$typeid = $_GET['id'];
						$midwife =mysqli_query($conn, "SELECT * FROM midwife WHERE midwife_id= '$typeid'");
						$row= mysqli_fetch_array($midwife);
						...
						echo '<div class="row fillbg l-border-radius-top">
								...
								<div class="row fillbg l-border-radius-bottom l-title-card">
								    <div class="col-4 col-md-3"><img src="directory/midwife/'.$row["logo"].'" class="img-fluid l-main-logo"></div>
								    <div class="col-5 col-md-6">
										...
										$followquery= mysqli_query($conn, "SELECT count(*) as totalfollow FROM mp_comments WHERE mp_id='$row[midwife_id]' and follow_status=1");
								    ...
								    echo '<div class="col-3 col-md-3 position-relative">
								        <p class="midwifetype position-absolute" style="right:10px"><span>'.$row['type'].'</span></p><br>
										...
										$followstatus = mysqli_query($conn, "SELECT * FROM mp_comments WHERE userid='$userid' AND mp_id='$row[midwife_id]' and follow_status=1");
										$followstatusrow = mysqli_fetch_array($followstatus);
						
										if ($followstatusrow && $followstatusrow['follow_status'] == 0) {
											echo '<button class="btn btn-success followbtn ml-2" value="'.$row["midwife_id"].'" style="font-size: 13px; padding: 0 5px;">Follow</button>';
										} else {
											echo '<button class="btn btn-secondary followbtn ml-2" value="'.$row["midwife_id"].'" style="font-size: 13px; padding: 0 5px;">Following</button>';
										} 
									echo '</div>
								</div>
								</div>';
									echo '<div class="row fillbg mt-1 l-border-radius py-2">
									    ...
									    echo '
									<div class="aboutmidwife" id="aboutmidwife">'.$row["about"].'</div>
									<p class="expand text-right" style="display:none">Read More</p>';
									/*$images = mysqli_query($conn, "SELECT * FROM mpgallery WHERE midwifeid = '$row[midwifeid]'");
									$count= mysqli_num_rows($images);
									if($count>0){
										echo '<br><h5 style="font-weight:bold">Gallery</h5>
											<div class="col">';
									while($row1= mysqli_fetch_array($images)){
										echo '<img src="directory/midwife/'.$row1["image_name"].'" class="img-fluid">';
									}
						 			echo '</div>';
									}*/
							echo	'</div>
						...
						?>
					<?php 
						$reviewquery = mysqli_query($conn, "SELECT comment from mp_comments WHERE mp_id ='$row[midwife_id]' AND userid='$userid'");
						...
						<div class="row p-2 fillbg l-border-radius" style="text-align:end; flex-flow: wrap">
							...
							<div class="" style="width:88%">
								<form method="POST" action="ajax/mp_review" id="reviewform">
									<input type="hidden" name="midwifeid" value="<?php echo $typeid;?>">
					...
					<div id="reviewlist" style="margin:0rem;">
						<?php 
							...
							$count =$numrows;
							if($count>0){
								echo '<div class="row fillbg "><p class="heading mt-1" style="font-weight:bold; font-size:12px">midwife REVIEW</p></div>';
			...
			<div class="modal-body mb-1 p-0">
				<div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:980px;height:480px;overflow:hidden;visibility:hidden;">
					...
					<div data-u="slides" style="cursor:default;position:relative;top:0px;left:0px;width:980px;height:380px;overflow:hidden; text-align:center">
						<?php
							$galquery = mysqli_query($conn, "SELECT * FROM mpgallery WHERE midwifeid= '$typeid'");
							while($galrow=mysqli_fetch_array($galquery)){
								echo ' 
										<div>
										<img data-u="image" src="directory/midwife/'.$galrow["image_name"].'">
										<img data-u="thumb" src="directory/midwife/'.$galrow["image_name"].'"   height="90">
										</div>
									';
							}
							?>     
			...
			<div class="modal-body m-auto mb-1 p-0">
				<?php
					if (strpos($videourl, 'iframe') !== false) {
						...
						echo '<video controls>
								  <source src="directory/midwife/video/'.$videourl.'" type="video/mp4">
...
<script>
			...
			var form = $("#reviewform");
	                form.validate({
	                    ...
	                    messages: {
							...
							rate:{
								required: "Give Rating to the midwife",
							},
	...
	$(document).on('click', '#btn_more', function(){  
	  ...
	  $(document).on('click', '.expand', function(){
		if($('#aboutmidwife').hasClass('aboutmidwife')){
			$('#aboutmidwife').removeClass('aboutmidwife');
			$('#aboutmidwife').css({'height':'auto', 'overflow':'none'});
			$('.expand').text('Read Less');
		}
		else{
			$('#aboutmidwife').addClass('aboutmidwife');
			$('.expand').text('Read More');
		}
	  });
	  if ($('#aboutmidwife')[0].scrollHeight >  $('#aboutmidwife').innerHeight()) {
...
<script>
	var follow="";
	async function update_follow(email, midwife_id, value){
	   let result = await $.ajax({
	        url: "ajax/followupdate",
	        data: {email : email, midwife_id:midwife_id, follow:'follow', value:value},
			type: "POST",
	        success: function(data) {
				//debugger;
	...
	if(sessionuser != ""){
		...
		var midwife_id=$(this).attr('value');
		var element = $(this);
		//update_like(sessionuser, articleid, 1).then(console.log); 
		async function main() {
		  var status = await update_follow(sessionuser, midwife_id, value)
