		...
		<div class="container-fluid p-0">
			<div class="row m-0 desktopviewArticle">
				...
				<div class="col-md-6">
					<?php $type = $_GET['type'];
						$typeid = $_GET['id'];
						$pharmacy =mysqli_query($conn, "SELECT * FROM pharmacy WHERE pharmacy_id= '$typeid'");
						$row= mysqli_fetch_array($pharmacy);
						...
						if(($type == 'pharmacy') && (mysqli_num_rows($pharmacy)>0)){
							$url=urlencode('https://www.mammypages.com/mppharmacy_details?type=pharmacy&id='.$row["pharmacy_id"]);
							$urltelegram=urlencode('https://www.mammypages.com/mppharmacy_details?type=pharmacy&id='.$row["pharmacy_id"].'&text='.$row["name"]);
						echo '<div class="row fillbg l-border-radius-top">
								<div class="text-center p-0">
								<img src="directory/pharmacy/'.$row["image"].'" class="img-fluid mb-2" style="width:100%; max-height:250px; border-radius: 15px;">
								<a href="pharmacys"><i class="bi bi-caret-left backbutton" data-toggle="tooltip" title="Back" data-placement="left" area-hidden="true"></i></a>
								</div>
							 </div>
								<div class="row fillbg l-border-radius-bottom l-title-card">
								    ...
								    <div class="col-5 col-md-6">
										...
										$followquery= mysqli_query($conn, "SELECT count(*) as totalfollow FROM mp_comments WHERE mp_id='$row[pharmacy_id]' and follow_status=1");
								    ...
								    echo '<div class="col-3 col-md-3 position-relative">
								        <p class="pharmacytype position-absolute" style="right:10px"><span>'.$row['type'].'</span></p><br>
									...
									while($row1= mysqli_fetch_array($images)){
										echo '<img src="directory/pharmacy/'.$row1["image_name"].'" class="img-fluid">';
									}
						 			echo '</div>';
									}*/
							echo	'</div>
						...
						?>
					<?php 
						$reviewquery = mysqli_query($conn, "SELECT comment from mp_comments WHERE mp_id ='$row[pharmacy_id]' AND userid='$userid'");
			...
			<div class="modal-body m-auto mb-1 p-0">
				<?php
					if (strpos($videourl, 'iframe') !== false) {
						...
						echo '<video controls>
								  <source src="directory/pharmacy/video/'.$videourl.'" type="video/mp4">
...
<script>
			...
			var form = $("#reviewform");
	                form.validate({
	                    ...
	                    messages: {
							...
							rate:{
								required: "Give Rating to the pharmacy",
							},
	...
	$(document).on('click', '#btn_more', function(){  
	  ...
	  $(document).on('click', '.expand', function(){
		if($('#aboutpharmacy').hasClass('aboutpharmacy')){
			$('#aboutpharmacy').removeClass('aboutpharmacy');
			$('#aboutpharmacy').css({'height':'auto', 'overflow':'none'});
			$('.expand').text('Read Less');
		}
		else{
			$('#aboutpharmacy').addClass('aboutpharmacy');
			$('.expand').text('Read More');
		}
	  });
	  if ($('#aboutpharmacy')[0].scrollHeight >  $('#aboutpharmacy').innerHeight()) {
