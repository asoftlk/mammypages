						...
						echo '<div class="row fillbg l-border-radius-top">
								...
								<div class="row fillbg l-border-radius-bottom l-title-card">
										...
										<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow" style="" > 
											...
											if ($followstatusrow && isset($followstatusrow['follow_status'])) {
												if ($followstatusrow['follow_status'] == 0) {
													echo '<button class="btn btn-success followbtn" value="'.$row["medical_id"].'" style="font-size: 13px; padding: 0 5px;">Follow</button>';
												} else {
													echo '<button class="btn btn-secondary followbtn" value="'.$row["medical_id"].'" style="font-size: 13px; padding: 0 5px;">Following</button>';
												}
											} else {
												echo '<button class="btn btn-success followbtn" value="'.$row["medical_id"].'" style="font-size: 13px; padding: 0 5px;">Follow</button>';
											}
									...
									echo '<div class="row fillbg mt-1 l-border-radius py-2">
									        ...
									        $galquery= mysqli_query($conn, "SELECT image_name FROM mpmedical_gallery WHERE medical_id= '$typeid' and image_name!=''");
									...
									/*$images = mysqli_query($conn, "SELECT * FROM mpmedical_gallery WHERE medical_id = '$row[medicalid]'");
			...
			<div class="modal-body mb-1 p-0">
				<div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:980px;height:480px;overflow:hidden;visibility:hidden;">
					...
					<div data-u="slides" style="cursor:default;position:relative;top:0px;left:0px;width:980px;height:380px;overflow:hidden; text-align:center">
						<?php
							$galquery = mysqli_query($conn, "SELECT * FROM mpmedical_gallery WHERE medical_id= '$typeid'");
							if (mysqli_num_rows($galquery) > 0) {
...
<script>
	...
	async function update_follow(email, medical_id, value){
	   let result = await $.ajax({
	        ...
	        data: {email : email, type_id:medical_id, follow:'follow', value:value},
			type: "POST",
	        success: function(data) {
				//debugger;
