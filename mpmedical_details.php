		...
		<div class="container-fluid p-0">
			<div class="row m-0 desktopviewArticle">
				...
				<div class="col-md-6">
					<?php $type = $_GET['type'];
						$typeid = $_GET['id'];
						$medical =mysqli_query($conn, "SELECT * FROM medical WHERE medical_id= '$typeid'");
						$row= mysqli_fetch_array($medical);
						...
						if(($type == 'medical') && (mysqli_num_rows($medical)>0)){
							$url=urlencode('https://www.mammypages.com/mpmedical_details?type=medical&id='.$row["medical_id"]);
							$urltelegram=urlencode('https://www.mammypages.com/mpmedical_details?type=medical&id='.$row["medical_id"].'&text='.$row["name"]);
						echo '<div class="row fillbg l-border-radius-top">
								<div class="text-center p-0">
								<img src="directory/medical/'.$row["image"].'" class="img-fluid mb-2" style="width:100%; max-height:250px; border-radius: 15px;">
								<a href="medical"><i class="bi bi-caret-left backbutton" data-toggle="tooltip" title="Back" data-placement="left" area-hidden="true"></i></a>
								</div>
							 </div>
								<div class="row fillbg l-border-radius-bottom l-title-card">
								    <div class="col-4 col-md-3"><img src="directory/medical/'.$row["logo"].'" class="img-fluid l-main-logo"></div>
								    <div class="col-5 col-md-6">
										...
										$followquery= mysqli_query($conn, "SELECT count(*) as totalfollow FROM mp_comments WHERE mp_id='$row[medical_id]' and follow_status=1");
								    ...
								    echo '<div class="col-3 col-md-3 position-relative">
								        <p class="medicaltype position-absolute" style="right:10px"><span>'.$row['type'].'</span></p><br>
										...
										$followstatus = mysqli_query($conn, "SELECT * FROM mp_comments WHERE userid='$userid' AND mp_id='$row[medical_id]' and follow_status=1");
										$followstatusrow = mysqli_fetch_array($followstatus);
						
										if ($followstatusrow && $followstatusrow['follow_status'] == 0) {
											echo '<button class="btn btn-success followbtn ml-2" value="'.$row["medical_id"].'" style="font-size: 13px; padding: 0 5px;">Follow</button>';
										} else {
											echo '<button class="btn btn-secondary followbtn ml-2" value="'.$row["medical_id"].'" style="font-size: 13px; padding: 0 5px;">Following</button>';
										} 
									echo '</div>
								</div>
								</div>';
									echo '<div class="row fillbg mt-1 l-border-radius py-2">
									        <h5 class="heading mt-1" style="font-weight:bold; font-size:12px">ABOUT medical';
									    ...
									    echo '
									<div class="aboutmedical" id="aboutmedical">'.$row["about"].'</div>
									<p class="expand text-right" style="display:none">Read More</p>';
									/*$images = mysqli_query($conn, "SELECT * FROM mpgallery WHERE medicalid = '$row[medicalid]'");
									$count= mysqli_num_rows($images);
									if($count>0){
										echo '<br><h5 style="font-weight:bold">Gallery</h5>
											<div class="col">';
									while($row1= mysqli_fetch_array($images)){
										echo '<img src="directory/medical/'.$row1["image_name"].'" class="img-fluid">';
									}
						 			echo '</div>';
									}*/
							echo	'</div>
						...
						?>
					<?php 
						$reviewquery = mysqli_query($conn, "SELECT comment from mp_comments WHERE mp_id ='$row[medical_id]' AND userid='$userid'");
						...
						<div class="row p-2 fillbg l-border-radius" style="text-align:end; flex-flow: wrap">
							...
							<div class="" style="width:88%">
								<form method="POST" action="ajax/mp_review" id="reviewform">
									<input type="hidden" name="medicalid" value="<?php echo $typeid;?>">
					...
					<div id="reviewlist" style="margin:0rem;">
						<?php 
							...
							$count =$numrows;
							if($count>0){
								echo '<div class="row fillbg "><p class="heading mt-1" style="font-weight:bold; font-size:12px">medical REVIEW</p></div>';
			...
			<div class="modal-body mb-1 p-0">
				<div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:980px;height:480px;overflow:hidden;visibility:hidden;">
					...
					<div data-u="slides" style="cursor:default;position:relative;top:0px;left:0px;width:980px;height:380px;overflow:hidden; text-align:center">
						<?php
							$galquery = mysqli_query($conn, "SELECT * FROM mpgallery WHERE medicalid= '$typeid'");
							while($galrow=mysqli_fetch_array($galquery)){
								echo ' 
										<div>
										<img data-u="image" src="directory/medical/'.$galrow["image_name"].'">
										<img data-u="thumb" src="directory/medical/'.$galrow["image_name"].'"   height="90">
										</div>
									';
							}
							?>     
			...
			<div class="modal-body m-auto mb-1 p-0">
				<?php
					if (strpos($videourl, 'iframe') !== false) {
						...
						echo '<video controls>
								  <source src="directory/medical/video/'.$videourl.'" type="video/mp4">
...
<script>
			...
			var form = $("#reviewform");
	                form.validate({
	                    ...
	                    messages: {
							...
							rate:{
								required: "Give Rating to the medical",
							},
	...
	$(document).on('click', '#btn_more', function(){  
	  ...
	  $(document).on('click', '.expand', function(){
		if($('#aboutmedical').hasClass('aboutmedical')){
			$('#aboutmedical').removeClass('aboutmedical');
			$('#aboutmedical').css({'height':'auto', 'overflow':'none'});
			$('.expand').text('Read Less');
		}
		else{
			$('#aboutmedical').addClass('aboutmedical');
			$('.expand').text('Read More');
		}
	  });
	  if ($('#aboutmedical')[0].scrollHeight >  $('#aboutmedical').innerHeight()) {
...
<script>
	var follow="";
	async function update_follow(email, medical_id, value){
	   let result = await $.ajax({
	        url: "ajax/followupdate",
	        data: {email : email, medical_id:medical_id, follow:'follow', value:value},
			type: "POST",
	        success: function(data) {
				//debugger;
	...
	if(sessionuser != ""){
		...
		var medical_id=$(this).attr('value');
		var element = $(this);
		//update_like(sessionuser, articleid, 1).then(console.log); 
		async function main() {
		  var status = await update_follow(sessionuser, medical_id, value)
