<?php
	...
	$galupdate = mysqli_query($conn, "SELECT * FROM mpstudio_gallery WHERE studio_id='$studioid'");
	if(mysqli_num_rows($galupdate)>0){
				While($galrow= mysqli_fetch_array($galupdate)){
					...
					foreach($galleryid as $value)
					{
						...
						if($key === false)
						{
						if(file_exists("../directory/studio/".$galrow['image_name'])){
							unlink("../directory/studio/".$galrow['image_name']);
							$artdelete = mysqli_query($conn, "DELETE FROM mpstudio_gallery  WHERE id = '$id1'");
						}
						else{
							$artdelete = mysqli_query($conn, "DELETE FROM mpstudio_gallery  WHERE id = '$id1'");
						}
						...
						else
						{
							...
							$target = $studioid.$key.$time.".".$ext;
									if (in_array($galleryimage, ['jpg', 'png', 'jpeg'])) {
										...
										move_uploaded_file($_FILES['galleryimage']['tmp_name'][$key], "../directory/studio/".$target);
										}
	...
	for ($x = 0; $x < $galcount; $x++) {
	           ...
	           //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
	               if (in_array($galleryimages, ['jpg', 'png', 'jpeg'])) {
	                   ...
	                   move_uploaded_file($_FILES['galleryimages']['tmp_name'][$x], "../directory/studio/".$target);
	                   }
	           }
	           else{
	               $galleryimage="";
	...
	$updatequery = "Update studio SET name='$name',speciality='$speciality',address='$address', map='$map', city='$city', mobile='$mobile',email='$email',whatsapp='$whatsapp',website='$website',type='$type', subtype='$subtype', working_hours='$working_hours', facebook='$facebook', instagram='$instagram', linkedin='$linkedin', status='$status',about='$about' ";		
		$featuredimage = $_FILES['featuredimage']['name'];
				...
				$logoimage = $_FILES['logoimage']['name'];
	            if($logoimage != ""){
					$removefile= mysqli_query($conn, "SELECT logo from studio WHERE id= '$id'");
					$filename = mysqli_fetch_array($removefile);
					if($filename['logo'] && file_exists("../directory/studio/".$filename['logo'])){
						unlink("../directory/studio/".$filename['logo']);
					}
	                ...
	                //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
	                    if (in_array($logoimage, ['jpg', 'png', 'jpeg'])) {
	                        ...
	                        move_uploaded_file($_FILES['logoimage']['tmp_name'], "../directory/studio/".$filetarget);
	                        }
				...
				if($galvideo != ""){
					$removevideo= mysqli_query($conn, "SELECT video from studio WHERE id= '$id'");
					$videoname = mysqli_fetch_array($removevideo);
					if($videoname['video'] && file_exists("../directory/studio/video/".$videoname['video'])){
						unlink("../directory/studio/video/".$videoname['video']);
					}
	                ...
	                //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
	                    if (in_array($galvideo, ['mp4', 'ogg', 'WebM'])) {
	                        ...
	                        move_uploaded_file($_FILES['galvideo']['tmp_name'], "../directory/studio/video/".$videotarget);
	                        }
							$updatequery .= ", video= '$videotarget' "; 
	            }
	            else{
