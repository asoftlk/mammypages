...
if(isset($_POST['sub-mid-branch'])){
    $midwifebranchid = "hbranch-".mt_rand(1000000,9999999);
    $midtime_id = "midbranchtime-".mt_rand(1000000,9999999);
	$midwife_id= filter_input(INPUT_POST, 'bmidwifename');
    $branchname = filter_input(INPUT_POST, 'branchname');
    
    $midwifeQuery = mysqli_query($conn, "SELECT * FROM midwife WHERE midwife_id = '$midwife_id'");
    $midwifeRow = mysqli_fetch_array($midwifeQuery);
    $midwifename = $midwifeRow['name'];
    $midwifetype = $midwifeRow['type'];
    ...
    $featuredimage = $_FILES['featuredimage']['name'];
        ...
        $featuretarget = "fea".$time.".".$ext1;
            ...
            if ($_FILES['featuredimage']['size'] > 30000000) { // file shouldn't be larger than 1Megabyte
                ...
                for ($x = 0; $x < count(array($galimages)); $x++) {
                    ...
                    //$image_link = "https://veramasa.com/udyogsadhna/images/".$target;
                        if (in_array($galimages, ['jpg', 'png', 'jpeg'])) {
                            ...
                            move_uploaded_file($_FILES['galimages']['tmp_name'][$x], "../directory/midwife/branch/".$target);
                            }
                    }
                    else{
                        $galimages="";
                ...
                $featureupload = move_uploaded_file($_FILES['featuredimage']['tmp_name'], "../directory/midwife/branch/".$featuretarget);
            }
        ...
        $logotarget = "logo".$time.".".$ext2;
            ...
            $logoupload = move_uploaded_file($_FILES['logoimage']['tmp_name'], "../directory/midwife/branch/".$logotarget);
            
        if($featureupload){
            $query= "INSERT INTO midwife_branch (midwifebranchid, mwtime_id, midwife_id, branchname, branchaddr, branchmap, branchcity, branchcont, branchwhatsapp, branchemail, branchweb, branchfb, branchinsta, branchln, status, about, videoembed, logoimage, featuredimage, galvideo)
            VALUES ('$midwifebranchid', '$midtime_id', '$midwife_id', '$midwifename - $branchname', '$branchaddr', '$branchmap', '$branchcity', '$branchcont', '$branchwhatsapp', '$branchemail', '$branchweb', '$branchfb', '$branchinsta', '$branchln', '$status', '$about', '$videoembed', '$logoimage', '$featuretarget', '$videotarget')";
            ...
            $workingTimesQuery = "INSERT INTO midwife_working_times (mwtime_id, midwife_id, midwife_type, branch_id, monday_open, monday_close, tuesday_open, tuesday_close, wednesday_open, wednesday_close, thursday_open, thursday_close, friday_open, friday_close, saturday_open, saturday_close, sunday_open, sunday_close) 
                        VALUES ('$midtime_id','$midwife_id', '$midwifetype', '$midwifebranchid', '$mon_open', '$mon_close', '$tue_open', '$tue_close', '$wed_open', '$wed_close', '$thu_open', '$thu_close', '$fri_open', '$fri_close', '$sat_open', '$sat_close', '$sun_open', '$sun_close')";
