...
<?php
    $id=$_GET['id'];
    $result = mysqli_query($conn,"SELECT * from saloon p LEFT JOIN saloon_working_times pwt ON pwt.saloon_id = p.saloon_id Where p.id='$id'");
...
$(document).ready(function() {
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Edit Saloon</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
		<div class="col-12">
			<a href="viewsaloon.php" class="btn btn-mammy">Cancel / Back</a>
		  </div><br><br>
          <!-- left column -->
          <div class="col-md-12">
            <!-- jquery validation -->
            <div class="card card-primary" style="border:none">
              <!-- form start -->
              <form id="quickForm" method="POST" action="updatesaloon" enctype="multipart/form-data">
			  <input type="hidden" name="id" value="<?php echo $row['id'];?>">
			  <input type="hidden" name="mpId" value="<?php echo $row['saloon_id'];?>">
                <div class="card-body">
					<div class="row">
                        <?php
                            if ($row["is_main"] === 'N') {
                                $mainSelectQuery = mysqli_query($conn, "SELECT * FROM saloon WHERE is_main = 'Y'");
                                if (!$mainSelectQuery) {
                        ...
                        <label class="required" for="mpname"><?php echo ($row["is_main"] === 'N') ? "Saloon Branch Name" : "Saloon Name" ?>
                                    </label>
					  <input type="text" name="mpname" class="form-control" id="mpname"
					value="<?php echo $row["name"]; ?>" placeholder="Saloon Name">
						</div>
						
						<div class="col-md-6 form-group">
						<label class="required" for="">Saloon Specialist In</label>
						<select class="form-control select2" name="specialist[]" id="specialist" multiple data-placeholder='--Select Speciality--'>
						<?php 
						$specialList=mysqli_query($conn, "SELECT * FROM saloon_speciality");
						$arraylist = array();
					...
					<div class="row">
					  ...
					  <input type="text" name="address" class="form-control" id="address"  value="<?php echo $row['address']; ?>" placeholder="Address">
						...
						<div class="col-md-6 form-group">
						<label class="required" for="mapLocation">Saloon Map location</label>
					...
					<div class="text-center">
						...
						<?php if(($row['logo']!="") && file_exists("../directory/saloon/".$row['logo'])){?>
							<p class="img-center" style="color:green;margin-bottom:0">Logo Preview</p>
								...
								<img src="../directory/saloon/<?php echo $row['logo'] ?>" class="img-fluid img-center" style="position:relative; width:200px; max-height: 180px; border:1px solid gray; margin-bottom:5px"/>
						<?php } ?>
						</div>
                    </div>
					...
					<div class="text-center">
						...
						<?php if(($row['image']!="") && file_exists("../directory/saloon/".$row['image'])){?>
							<p class="img-center" style="color:green;margin-bottom:0">Featured Image Preview</p>
								...
								<img src="../directory/saloon/<?php echo $row['image'] ?>" class="img-fluid img-center" style="position:relative; width:200px; max-height: 180px; border:1px solid gray; margin-bottom:5px"/>
						<?php } ?>
						</div>
                    </div>
					...
					<?php
						$galquery= mysqli_query($conn, "SELECT * FROM mpsaloon_gallery WHERE saloon_id= '$row[saloon_id]'");
						$i=0;
						...
						if(($galrow['image_name']!="") && file_exists("../directory/saloon/".$galrow['image_name'])){
							echo '<p class="img-center" style="color:green;margin-bottom:0">Gallery Image Preview</p>
								...
								<img src="../directory/saloon/'.$galrow["image_name"].'"  class="img-fluid img-center" style="position:relative; width:200px; max-height: 180px; border:1px solid gray; margin-bottom:5px"/>';
						} 
					echo	'</div>
