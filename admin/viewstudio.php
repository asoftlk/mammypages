...
<?php
	...
	{
	$id = $_GET['del_id'];
	$data1 = mysqli_query($conn,"select * from studio where id = '$id'");
	if(mysqli_num_rows($data1)>0){
		$row = mysqli_fetch_array($data1);
		if(file_exists("../directory/studio/".$row['logo'])){
		...
		while($galleryrow = mysqli_fetch_array($galleryquery)){
			...
			unlink("../directory/studio/".$galleryrow['image_name']);
			}
		}
		mysqli_query($conn, "DELETE FROM mpstudio_gallery WHERE studio_id='$row[studio_id]'");
		mysqli_query($conn, "DELETE FROM studio WHERE id=$id");
        mysqli_query($conn, "DELETE FROM studio_working_times WHERE studio_id='$row[studio_id]'");
		echo '<script>alert("Deleted Successfully");window.location.href="viewstudio";</script>';  
	...
	if(isset($_GET['id'])) 
	{
		$uid = $_GET['id'];
		 $value = $_GET['value'];
		 if($value>0){
			$data =mysqli_query($conn, "SELECT * FROM `studio` WHERE `priority` = '$value'");
			echo mysqli_num_rows($data);
			if(mysqli_num_rows($data)>0){
				$studioidrow = mysqli_fetch_array($data);
				echo '<script>removeReg("This priority is already assigned to '.$studioidrow["name"].'('.$studioidrow["studio_id"].')", "error");</script>';
			}
			else{
				$updatedata =mysqli_query($conn, "UPDATE studio SET priority = '$value' WHERE id='$uid'");
				if($updatedata){
				echo '';
...
<div class="content-wrapper">
	...
	<section class="content">
		<div class="container-fluid">
			<div class="row">
				<div class="col-12">
					<a href="studio.php" class="btn btn-mammy float-right">+ Add Studio</a>
				</div>
				<br><br>
				<div class="col-12">
					<div class="card" >
			...
			<div class="modal-body">
				<form method="post">
					<div class="container">
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label>Studio Id:</label><input type="text" class="form-control" id="studio_id">
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label>Studio Map location:</label>
						...
						<!--a href="viewstudio.php?update_id=26"><button type="button" class="btn btn-sm btn-secondary">Update</button></a-->
				</form>
				</div>
