...
<?php
	...
	{
	$id = $_GET['del_id'];
	$data1 = mysqli_query($conn,"select * from midwife where id = '$id'");
	if(mysqli_num_rows($data1)>0){
		$row = mysqli_fetch_array($data1);
		if(file_exists("../directory/midwife/".$row['logo'])){
	...
	if(isset($_GET['id'])) 
	{
		$uid = $_GET['id'];
		 $value = $_GET['value'];
		 if($value>0){
			$data =mysqli_query($conn, "SELECT * FROM `midwife` WHERE `priority` = '$value'");
			echo mysqli_num_rows($data);
			if(mysqli_num_rows($data)>0){
				$midwifeidrow = mysqli_fetch_array($data);
				echo '<script>removeReg("This priority is already assigned to '.$midwifeidrow["name"].'('.$midwifeidrow["midwife_id"].')", "error");</script>';
			}
			else{
				$updatedata =mysqli_query($conn, "UPDATE midwife SET priority = '$value' WHERE id='$uid'");
				if($updatedata){
				echo '';
			...
			<div class="modal-body">
				<form method="post">
					<div class="container">
						...
						<!--a href="viewmidwife.php?update_id=26"><button type="button" class="btn btn-sm btn-secondary">Update</button></a-->
				</form>
				</div>
	           ...
	           for(var key in data1){
	               if($("#" + key).length != 0){
	                   if(key == 'logo')
	                       {
	                       if(data1[key]){
	                         $('#'+key).attr('src', '../directory/midwife/'+data1[key]);  
