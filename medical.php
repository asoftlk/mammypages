         ...
         <div class="top-menu">
			...
			<?php $medical =mysqli_query($conn, "SELECT * FROM medical WHERE priority > 0 ORDER BY priority LIMIT 5");
					...
					echo '<div class="row m-0 priority-list" style="border-bottom: 1px solid #f4f4f4 ;">
							...
							<div>
								<a href="mpconnect/medical/' .urlencode(str_replace(' ', '_', $row["name"])). '"><img src="directory/medical/'.$row['logo'].'" class="img-fluid" style="max-height:5rem"></a>
							</div>
							</div>
							<div class="col-md-9 pl-0" style="margin:1rem 0">
							<div class="d-flex">
                            <p class="text"><a href="mpconnect/medical/' .urlencode(str_replace(' ', '_', $row["name"])). '" class="namehref"><p class="text-heading">&nbsp;'.$row["name"].'</p></a>
                            ...
                            <p class="text"><img src="assets/images/placeholder.png" class="img-fluid" style="border-radius:10px; width:16px">&nbsp;'.$row["address"].'</P>                           
                                ...
                                <a href="mpconnect/medical/' .urlencode(str_replace(' ', '_', $row["name"])). '" type="button" class="btn btn-success p-1 ml-auto" style="font-size:12px; height:28px">View&nbsp;medical</a>
                            </div>
                         </div>   
						</div>';
					}
                ...
                success: function(data) {
                    ...
                    if (result.data.length > 0) {
                        ...
                        result.data.forEach(function(medical) {
                            ...
                            html += '<a href="mpconnect/medical/' + encodedName + '">';
                            ...
                            html += '<a href="mpconnect/medical/' + encodedName + '" type="button" class="btn btn-success p-1 ml-auto" style="font-size:12px; height:28px">View&nbsp;medical</a>';
                            html += '</div></div></div>';
                        });
                        $("#load_data").html(html);
                    } else {
