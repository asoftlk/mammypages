   ...
   <div class="col-md-6">
      <div class="article-sec">
        ...
        <form action="" method="POST">
            <input class="form-control form-control-sm" type="search" name="searchMedical" id="searchMedical" placeholder="Search medical" aria-label="Search">
            ...
            <div class="d-flex justify-content-start">
                <label class="select">
                    <select name="speciality"class="filter-box form-select form-select-sm"  id="speciality">
                        ...
                        <?php
                            $specialityQuery = mysqli_query($conn, "SELECT DISTINCT speciality FROM medical");
                ...
                <label class="select">
                    <select name="city"  class="filter-box form-select form-select-sm" id="city">
                        ...
                        <?php
                            $cityQuery = mysqli_query($conn, "SELECT DISTINCT city FROM medical");
         ...
         <div class="top-menu">
			...
			<?php $medical =mysqli_query($conn, "SELECT * FROM medical INNER JOIN medical_working_times wt ON wt.medical_id = medical.medical_id WHERE priority > 0 ORDER BY priority LIMIT 5");
					$count = 0;
					...
					while($row=mysqli_fetch_array($medical)){
                        ...
                        echo '<div class="row m-0 priority-list sort-item">
							...
							<div>
                                <a href="mpconnect/medical/' . urlencode(str_replace(' ', '_', $row["name"])) . '">
                                    <img src="directory/medical/' . $row['logo'] . '" class="img-fluid" style="max-height:5rem">
                                </a>
							</div>
							</div>
							...
							<div class="d-flex">
                            <p class="text">
                            <a href="javascript:void(0)" onclick="location.href=\'mpconnect/medical/' . urlencode(str_replace(' ', '_', $row['name'])) . '\'" style="color: inherit; text-decoration: none;" class="text-heading text-capitalize">
                            <p class="text-heading text-capitalize">&nbsp;'.$type_name_head.'</p></a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#searchMedical, #speciality, #type, #city').on('input change keyup', function() {
                ...
                success: function(data) {
                    ...
                    if (result.data.length > 0) {
                        ...
                        result.data.forEach(function(medical) {
                            ...
                            var encodedName = encodeURIComponent(medical.name.replace(/\s+/g, '_'));
                            var medicalId = medical.medical_id;
                            ...
                            if (medical[isOpen24Key] === 'Y') {
                                isOpen = '<span class="text-success text mr-1" style="float:right">Open</span>';
                            } else if (medical[extendsKey] === 'Y') {
                            ...
                            html += '<a href="mpconnect/medical/'+ encodedName +'"><img src="directory/medical/' + medical.logo + '" class="img-fluid sort-item-img" style="max-height:5rem"></a>';
                            html += '</div>';
