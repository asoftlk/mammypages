...
$type = $_POST['type']; 
//sleep(1);  
//$sql = "SELECT * FROM medicals WHERE id > '.$_POST[last_video_id].' LIMIT 2"; 
$sql = "SELECT * FROM medical WHERE priority = 0 LIMIT 10 OFFSET ".$_POST['count']; 
		...
		for($i=0; $i< count($specialityarray); $i++){
          ...
          $output .= '  
               <div class="row m-0" style="border-bottom: 1px solid #f4f4f4 ;">
							...
							$output .= '</div><a href="mpmedical_details.php?type=medical&id='.$row["medical_id"].'" class="namehref"><p class="text-heading">&nbsp;'.$row["name"].'</p></a>
                            ...
                            <p class="text"><img src="assets/images/placeholder.png" class="img-fluid" style="border-radius:10px; width:16px">&nbsp;'.$row["address"].'</P>                           
                                ...
                                <a href="mpmedical_details.php?type=medical&id='.$row["medical_id"].'" type="button" class="btn btn-success p-1" style="font-size:12px; height:28px">View&nbsp;medical</a>
                            </div>
                         </div>   
						</div>';  
     }  
