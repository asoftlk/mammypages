...
if (isset($_POST["search"])) {
    ...
    if (mysqli_num_rows($query) > 0) {
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            $html .= '<li><a class="title-list" href="mpconnect/medical/' . urlencode(str_replace(' ', '_', $row["name"])) . '" style="text-decoration:none; color:black"><div class="d-flex"><img src="directory/medical/'.$row["logo"].'" style="width:50px; height:40px; object-fit:cover; margin-right:5px; border-radius:5px"><p class="mb-0">'.$type_name_head.'</p></div></a></li>';
            $list[] = $row;
        }
    } else {
