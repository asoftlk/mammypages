...
if (isset($_POST["search"])) {
    ...
    if (mysqli_num_rows($query) > 0) {
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            $ratingQuery = mysqli_query($conn, "SELECT SUM(rating) AS total, COUNT(rating) as count FROM mp_comments WHERE mp_id= '$medicalId'");
            ...
            if (isset($row['main_id']) && !empty($row['main_id']) && $row['main_id'] != 0) {
                $mainId = $row['main_id'];
                $nameQuery = mysqli_query($conn, "SELECT `name` FROM `medical` WHERE `id` = '$mainId'");
                if ($nameQuery) {
            ...
            $html .= '<li><a class="title-list" href="mpconnect/medical/' . urlencode(str_replace(' ', '_', $row["name"])) . '" style="text-decoration:none; color:black"><div class="d-flex"><img src="directory/medical/'.$row["image"].'" style="width:50px; height:40px; object-fit:cover; margin-right:5px; border-radius:5px"><p class="mb-0">'.$type_name_head.'</p></div></a></li>';
            $list[] = $row;
        }
    } else {
        $html .= '<li>No Medical Clinic Found</li>';
    }
